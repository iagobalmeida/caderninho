...
from caderninho.src.utils import redirect_url_for
...
async def usuario_autenticar(
    session: Session, request: Request, email: str, senha: str
) -> bool:
...
async def request_login(
    ...
    try:
        await authenticate(
            session, request, email, senha, lembrar_de_mim=lembrar_de_mim
        )
...
def header_authorization(request: Request) -> str:
    ...
    if (
        request.state.auth.expires
        and datetime.fromtimestamp(request.state.auth.expires) <= datetime.now()
    ):
        request.session.clear()
