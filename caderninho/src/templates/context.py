...
from caderninho.src.schemas.docs import get_sobre_essa_pagina_html
from caderninho.src.templates.chartjs import chart_fluxo_caixa_config
from caderninho.src.templates.filters import (
    templates_global_material_symbol as material_symbol,
)
from caderninho.src.utils import url_incluir_query_params
...
class Context(TypedDict):
    ...
    def factory_navbar_link(
        ...
        return Context.Navbar.Link(
            ...
            url=nav_url,
            active=(
                request.url == base_nav_url or str(base_nav_url) in str(request.url)
            ),
        )
...
ADMIN_NAVBAR_LINKS = [("Logs", "android", "get_admin_logs")]


async def update_context_with_chart(
    auth_session: AuthSession, db_session, base_context: dict
):
    ...
    labels = [c[0] for c in chart_datasets]
    ...
    base_context.update(
        chart_resumo_caixa_data_atualizacao=chart_datasets_cached["created_at"]
    )
    base_context.update(chart_resumo_caixa_data_inicial=datas[0])
...
async def get_context(
    ...
    session=None,
    context: dict = None,
    navbar_links: list = BASE_NAVBAR_LINKS,
):
    ...
    base_context = Context(
        ...
        header=Context.Header(pretitle="CADERNINHO", title="Home", symbol="home"),
    ...
    if auth_session:
        ...
        db_receitas, _, _ = await repository.get(
            auth_session=auth_session,
            db_session=session,
        ...
        base_context = await update_context_with_chart(
            auth_session=auth_session, db_session=session, base_context=base_context
        )
