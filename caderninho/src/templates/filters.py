...
def __unit_converter(
    input: float,
    unity: str = "g",
    converter_kg: bool = False,
    converter_kg_sempre: bool = False,
):
...
def status_html(classname: str, content: str, material_symbol: str = None):
    material_symbol_html = (
        f'<span class="material-symbols-outlined"> {material_symbol} </span>'
        if material_symbol
        else ""
    )
    return f"""
...
def templates_filter_format_stock(
    input: float,
    converter_kg: bool = False,
    converter_kg_sempre: bool = False,
    icon_positive: str = "check",
    icon_zero: str = "more_horiz",
    icon_negative: str = "close",
    unity: str = "g",
):
...
def templates_filter_format_stock_movement(
    input: float, converter_kg: bool = False, converter_kg_sempre: bool = False
):
    return templates_filter_format_stock(
        input,
        icon_positive="arrow_upward",
        icon_negative="arrow_downward",
        converter_kg=converter_kg,
        converter_kg_sempre=converter_kg_sempre,
    )


def templates_filter_format_quantity(
    input: float,
    converter_kg: bool = False,
    converter_kg_sempre: bool = False,
    unity: str = "g",
...
BASE_GLOBALS = [("material_symbol", templates_global_material_symbol)]


def init_filters(
    templates: Jinja2Templates, filters: List[Tuple[str, callable]] = BASE_FILTERS
):
...
def init_globals(
    templates: Jinja2Templates, _globals: List[Tuple[str, callable]] = BASE_GLOBALS
):
