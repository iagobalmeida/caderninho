...
from caderninho.src.routers.caixa_movimentacoes import (
    router as router_caixa_movimentacoes,
)
from caderninho.src.routers.estoques import router as router_estoques
from caderninho.src.routers.insumos import router as router_insumos
from caderninho.src.routers.organizacao import router as router_organizacao
from caderninho.src.routers.paginas import router as router_paginas
from caderninho.src.routers.receitas import router as router_receitas
from caderninho.src.routers.scripts import router as router_scripts
from caderninho.src.scripts.seed import main as seed
from caderninho.src.templates import render
from caderninho.src.utils import redirect_url_for, url_incluir_query_params
...
app = fastapi.FastAPI(on_startup=[db.init, seed])
...
app.mount("/static", StaticFiles(directory="caderninho/src/static"), name="static")
...
async def get_index(request: fastapi.Request):
    return await render(
        request,
        "index.html",
        context={"planos": PLANOS_DATA, "body_class": "index", "data_bs_theme": "auto"},
    )


@app.get("/app", include_in_schema=False)
async def get_app_index(
    request: fastapi.Request,
    message: str = fastapi.Query(None),
    error: str = fastapi.Query(None),
):
    return await render(
        request,
        "autenticacao/login.html",
        context={"message": message, "error": error, "data_bs_theme": "auto"},
    )
...
async def post_registrar(
    ...
    db_organizacao = await repository.create(
...
async def get_recuperar_senha(
    ...
    return await render(
        request,
        "autenticacao/recuperar_senha.html",
        context={"message": message, "error": error, "data_bs_theme": "auto"},
    )


@app.post("/app/recuperar_senha", include_in_schema=False)
async def post_recuperar_senha(
    request: fastapi.Request,
    ...
    auth_session = getattr(request.state, "auth", None)
    db_usuario, _, _ = await repository.get(
        auth_session=auth_session,
        db_session=session,
        entity=repository.Entities.USUARIO,
        filters={"email": email},
    ...
    if not db_usuario:
        return redirect_url_for(
            request,
            "get_recuperar_senha",
            error="Não foi encontrado usuário com esse email",
        )
    ...
    return await render(
        ...
        "autenticacao/login.html",
        context={
            "message": f"Senha enviada para: {db_usuario.email}",
            "error": error,
            "data_bs_theme": "auto",
        },
    )
...
async def integrity_error_exception_handler(
    request: fastapi.Request, ex, redirect_to: str = None
):
    logger.exception(ex)
    if not redirect_to:
