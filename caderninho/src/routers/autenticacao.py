...
from caderninho.src import auth, db
...
async def post_index(
    ...
    lembrar_de_mim: bool = fastapi.Form(False),
    session: db.AsyncSession = db.DBSESSAO_DEP,
):
    return await auth.request_login(
        session, request, email=email, senha=senha, lembrar_de_mim=lembrar_de_mim
    )
...
async def post_authenticate(
    ...
    return await auth.usuario_autenticar(
        session, request=request, email=email, senha=senha
    )
...
async def post_perfil(
    request: fastapi.Request,
    payload: inputs.UsuarioAtualizar = fastapi.Form(),
    session: db.AsyncSession = db.DBSESSAO_DEP,
):
...
async def post_atualizar_senha(
    request: fastapi.Request,
    payload: inputs.AtualizarSenha = fastapi.Form(),
    session: db.AsyncSession = db.DBSESSAO_DEP,
):
