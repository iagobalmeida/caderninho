...
from caderninho.src.tests.mocks import MOCK_ORGANIZACAO
...
async def test_post_organizacao_configuracoes(client):
    await autenticar(client)
    response = await client.post(
        "/app/organizacao/configuracoes",
        data={"id": str(MOCK_ORGANIZACAO.id), "converter_kg": True},
    )
    assert response.status_code == 200
...
async def test_post_organizacao_usuarios_criar(client):
    await autenticar(client)
    response = await client.post(
        "/app/organizacao/usuarios/criar",
        data={
    ...
    usuarios_nomes = [
        json.loads(tr.get("data-bs-payload"))["nome"] for tr in dom_table_trs
    ]
