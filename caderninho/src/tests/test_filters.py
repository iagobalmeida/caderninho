...
def test_filters_markdown():
    ...
    assert (
        filters.templates_filter_markdown("`Teste` __funciona!__") == markdown_expected
    )
...
def test_filters_strftime():
    ...
    assert (
        filters.templates_filter_strftime(datetime(2025, 1, 1, 10, 10))
        == strftime_expected
    )


def test_filters_material_symbol():
    material_symbol_expected = (
        '<span class="material-symbols-outlined me-1 teste">teste</span>'
    )
    assert (
        filters.templates_global_material_symbol("teste", "teste")
        == material_symbol_expected
    )
