...
def __get_organizacao(test_client):
    response = test_client.get('/app/organizacao')
    soup = BeautifulSoup(response.content, 'html.parser')
    return soup


def test_get_organizacao_index(client):
    autenticar(client)
    soup = __get_organizacao(client)
...
def test_post_organizacao_index(client):
    autenticar(client)
    response = client.post('/app/organizacao', data={
...
def test_post_organizacao_configuracoes(client):
    autenticar(client)
    response = client.post('/app/organizacao/configuracoes', data={
        'id': MOCK_ORGANIZACAO.id,
        'converter_kg': True
    })
    assert response.status_code == 200
...
def test_post_organizacao_usuarios_criar(client):
    autenticar(client)
    response = client.post('/app/organizacao/usuarios/criar', data={
...
def test_post_organizacao_usuarios_criar_senhas_nao_batem(client):
    autenticar(client)
    response = client.post('/app/organizacao/usuarios/criar', data={
        'nome': 'Outro Zé do Teste',
        'email': 'outro@email.com',
