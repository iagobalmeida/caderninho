...
router = fastapi.APIRouter(prefix='/app/caixa_movimentacoes', dependencies=[auth.HEADER_AUTH])


@router.get('/', include_in_schema=False)
async def get_caixa_movimentacoes_index(request: fastapi.Request, page: int = fastapi.Query(1), db_session: Session = DBSESSAO_DEP):
...
async def post_caixa_movimentacao_excluir(request: fastapi.Request, selecionados_ids: str = fastapi.Form(), db_session: Session = DBSESSAO_DEP):
...
async def post_caixa_movimentacoes_marcar_recebido(request: fastapi.Request, selecionados_ids: str = fastapi.Form(), session: Session = DBSESSAO_DEP):
    auth_session = getattr(request.state, 'auth', None)
    if selecionados_ids:
        for id in selecionados_ids.split(','):
            await repository.update(auth_session=auth_session, db_session=session, entity=repository.Entities.CAIXA_MOVIMENTACAO, filters={'id': id}, values={'recebido': True})
    return redirect_back(request, message='CaixaMovimentacao(s) atualizada(s) com sucesso!')


@router.post('/marcar/pendente', include_in_schema=False)
async def post_caixa_movimentacoes_marcar_pendente(request: fastapi.Request, selecionados_ids: str = fastapi.Form(), session: Session = DBSESSAO_DEP):
    auth_session = getattr(request.state, 'auth', None)
    if selecionados_ids:
        for id in selecionados_ids.split(','):
            await repository.update(auth_session=auth_session, db_session=session, entity=repository.Entities.CAIXA_MOVIMENTACAO, filters={'id': id}, values={'recebido': False})
    return redirect_back(request, message='CaixaMovimentacao(s) atualizada(s) com sucesso!')
