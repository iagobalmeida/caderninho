...
class PlanoBeneficio(BaseModel):
    ...
    def __str__(self):
        conteudo = re.sub(r'\b\d{1,3}(?:\.\d{3})*\b', lambda m: f'<b>{m.group()}</b>', self.conteudo)
        attrs = ''
        classnames = ''
        if self.classname:
...
PLANOS_DATA = {
    ...
    'Intermediário': Plano(
        ...
        card_beneficios=[
            ...
            PlanoBeneficio(conteudo='Exportação de Caixa e Estoque', classname='primary')
        ]
    ),
    'Avançado': Plano(
        nome='Avançado',
        ...
        card_beneficios=[
            ...
            PlanoBeneficio(conteudo='Até 40.000 Movimentações/mês'),
            PlanoBeneficio(conteudo='Exportação de Caixa e Estoque', classname='success')
        ]
    )
}
